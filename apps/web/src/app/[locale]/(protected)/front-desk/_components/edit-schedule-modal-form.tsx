...
import { CheckIcon, ChevronsUpDownIcon, PencilIcon } from "@repo/ui/icons"
...
import type { EditScheduleFields } from "../_validations/new-schedule-validation"
import { SECOND } from "~/app/[locale]/_shared/utils/constants"
import { getStatuses } from "~/app/[locale]/_shared/utils/status"
import { getEditScheduleSchema } from "../_validations/new-schedule-validation"
...
interface Props {
  appointment: NonNullable<FunctionReturnType<typeof api.receptionQueue.get>>
}

export function EditScheduleModalForm(props: Props) {
  const t = useI18n()
  const { appointment } = props

  const form = useForm<EditScheduleFields>({
    resolver: zodResolver(getEditScheduleSchema(t)),
  ...
  async function onSubmit(values: EditScheduleFields) {
    ...
    const payload = {
      receptionQueueId: appointment.receptionQueueId,
    ...
    } satisfies FunctionArgs<typeof api.receptionQueue.editSchedule>
